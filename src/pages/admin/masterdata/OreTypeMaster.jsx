...
import { oreTypesMeta } from "../../../data/adminmasterdatafiles/oreTypes";
...
function toApi(form) {
  return {
    ore_type: form.name || null,
    ore_type_desc: form.description || null,
...
function OreTypeForm({ initialData, onSave, onCancel, isEditing, saving }) {
    ...
    <form onSubmit={handleSubmit} className="space-y-4 3xl:space-y-6">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 3xl:gap-5">
        <div>
          ...
          <input type="text" placeholder="e.g. High Grade Hematite"
      ...
      <div>
        ...
        <textarea placeholder="Brief description of the ore type..."
      ...
      <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-100">
        ...
        <button type="submit" disabled={saving}
          ...
          {saving ? "Saving…" : isEditing ? "Update Ore Type" : "Add Ore Type"}
        </button>
      </div>
    </form>
  );
...
export default function OreTypeMaster() {
  ...
  const [oreTypes, setOreTypes] = useState([]);
  ...
  const loadOreTypes = useCallback(async () => {
    setLoading(true);
    try {
      const filters = search.trim() ? { ore_type: search.trim() } : {};
      const res = await fetchOreTypes(filters);
      const results = res?.results ?? res ?? [];
      setOreTypes(results.map(toUi));
      setTotalCount(res?.count ?? results.length);
    } catch (err) {
      showToast(err.message || "Failed to load ore types.", "error");
    } finally {
      setLoading(false);
    }
  }, [search]);
  ...
  useEffect(() => {
    if (!isAddPage && !isEditPage) loadOreTypes();
  }, [isAddPage, isEditPage, loadOreTypes]);

  const sorted = [...oreTypes].sort((a, b) => {
  ...
  async function handleAddSave(formData) {
    setSaving(true);
    try {
      await createOreType(toApi(formData));
      showToast(`"${formData.name}" added successfully.`);
      navigate(baseRoute);
      loadOreTypes();
    } catch (err) {
      showToast(err.message || "Failed to add ore type.", "error");
    } finally {
      setSaving(false);
    }
  }
  ...
  async function handleEditSave(formData) {
    setSaving(true);
    try {
      await updateOreType(editingItem.id, toApi(formData));
      showToast(`"${formData.name}" updated successfully.`);
      setEditingItem(null);
      navigate(baseRoute);
      loadOreTypes();
    } catch (err) {
      showToast(err.message || "Failed to update ore type.", "error");
    } finally {
      setSaving(false);
    }
  }
  ...
  async function confirmStatusToggle() {
    ...
    try {
      await patchOreType(statusToggleItem.id, { status: newStatus });
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          <p className="text-[14px] text-slate-600">Select an ore type from the list to edit.</p>
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <h2 className="text-[24px] sm:text-[28px] 3xl:text-[34px] font-bold text-slate-800">
              {isEditPage ? "Edit Ore Type" : "Add New Ore Type"}
            </h2>
            <p className="mt-1 text-[14px] 3xl:text-[17px] text-slate-500">Fill in the ore type details.</p>
          </div>
    ...
    <div className="space-y-6 3xl:space-y-8">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-[14px] 3xl:text-[17px] text-slate-500">{oreTypesMeta.subtitle}</p>
        </div>
          ...
          <table className="w-full" data-print-table>
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50/60">
                ...
                <th className="px-5 py-3.5 text-left text-[11px] 3xl:text-[13px] font-bold tracking-[0.06em] text-slate-500 uppercase">
                  {renderSortButton("Ore Type Name", "name")}
                </th>
              ...
              {loading ? (
                <tr>
                  <td colSpan={5} className="px-5 py-12 text-center">
                    <p className="text-[14px] text-slate-400 animate-pulse">Loading ore types…</p>
                  </td>
                </tr>
                ...
                paginatedData.map((ore) => (
                  <tr key={ore.id} className="hover:bg-slate-50/60 transition-colors group">
                    <td className="px-5 py-4 3xl:px-6 3xl:py-5">
                      <span className="text-[13px] 3xl:text-[15px] font-semibold text-blue-600">{ore.id}</span>
                    </td>
                    <td className="px-5 py-4 3xl:px-6 3xl:py-5">
                      <span className="text-[13px] 3xl:text-[15px] font-semibold text-slate-800">{ore.name || "—"}</span>
                    </td>
                    <td className="px-5 py-4 3xl:px-6 3xl:py-5 hidden md:table-cell">
                      <span className="text-[13px] 3xl:text-[15px] text-slate-500 truncate max-w-50 block">
                        {ore.description || "—"}
                      </span>
                    </td>
                    <td className="px-5 py-4 3xl:px-6 3xl:py-5">
                    ...
                    <td className="px-5 py-4 3xl:px-6 3xl:py-5">
                      <div className="flex items-center justify-end gap-2 opacity-60 group-hover:opacity-100 transition-opacity">
                        <button onClick={() => { setEditingItem(ore); navigate(editRoute); }}
                          disabled={ore.status === "inactive"}
                          className={`flex h-8 w-8 3xl:h-10 3xl:w-10 items-center justify-center rounded-lg transition-colors ${ore.status === "inactive" ? "cursor-not-allowed text-slate-300" : "text-slate-400 hover:bg-blue-50 hover:text-blue-600"}`}
                          title={ore.status === "inactive" ? "Enable to edit" : "Edit"}>
                          <EditIcon />
                        </button>
                        <button onClick={() => { setStatusToggleItem(ore); setIsStatusToggleOpen(true); }}
                          className={`flex h-8 w-8 3xl:h-10 3xl:w-10 items-center justify-center rounded-lg transition-colors ${ore.status === "inactive" ? "text-emerald-500 hover:bg-emerald-50 hover:text-emerald-600" : "text-slate-400 hover:bg-amber-50 hover:text-amber-600"}`}
      ...
      <ConfirmDialog
        ...
        title={statusToggleItem?.status === "inactive" ? "Enable Ore Type" : "Disable Ore Type"}
